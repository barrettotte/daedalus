...
{#if $minimalView}
  <div class="card minimal" class:dragging={isDragging} class:focused={focused} draggable="true" role="button"
  ...
  >
    ...
    <span class="minimal-title" title={meta.title}>{meta.title}</span>
  </div>
{:else}
  <div class="card" class:dragging={isDragging} class:focused={focused} class:has-icon={!!meta.icon}
    draggable="true" role="button" tabindex="0" ondragstart={handleDragStart} ondragend={handleDragEnd}
    onclick={openDetail} oncontextmenu={handleContextMenu} onkeydown={e => e.key === 'Enter' && openDetail()}
  >
    <div class="top-row">
